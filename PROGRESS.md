...
## 🔧 Phase 11: 전략 빌더 & 포트폴리오 백테스트 (진행 중)

### 목표
- 노코드 전략 빌더로 포트폴리오 전략 생성
- 종목 선정 조건 → `select_universe()` 메서드 자동 생성
- 단일 종목 / 포트폴리오 전략 통합 지원

### ✅ 완료된 작업 (2025-12-08)

#### 11.1 전략 빌더 백엔드
- [x] `_has_stock_selection_criteria()` - 종목 선정 조건 확인
- [x] `_generate_select_universe_method()` - select_universe 메서드 생성
- [x] `generate_strategy_code()` - 포트폴리오 전략 코드 생성
- [x] `save_strategy()` - UPDATE/INSERT 로직 (strategy_id 기반)
- [x] `list_strategies()` - is_portfolio 필드 추가

#### 11.2 전략 빌더 프론트엔드
- [x] URL 파라미터로 수정 모드 지원 (`?edit=3`)
- [x] `editingStrategyId` 상태 관리
- [x] `loadStrategy()` - 전략 로드
- [x] `handleSave()` - strategy_id 포함 저장

#### 11.3 백테스트 통합
- [x] `/api/backtest/portfolio` - 포트폴리오 전용 엔드포인트
- [x] 전략 빌더 전략 동적 로딩 (DB에서 python_code 로드)
- [x] `BacktestPage.tsx` - is_portfolio 자동 감지
- [x] 포트폴리오 전략 시 symbol 입력란 자동 숨김

#### 11.4 데이터 수집
- [x] LS증권 API 재무정보 조회 (t3320)
- [x] 종목 코드 형식 수정 (A005930 → 005930)
- [x] 50개 종목 재무 데이터 업데이트 (PER, PBR, ROE)

### 🐛 현재 이슈

**문제**: 전략 ID 3 코드 생성 시 syntax error (187번째 줄)

**원인**: `_generate_select_universe_method()`에서 `.filter()` 메서드 체이닝 줄바꿈 오류
```python
# 문제 코드
query = db.query(StockMasterModel.symbol).filter(...)      
.filter(...)  # ← syntax error
```

**해결**: 각 filter를 별도 라인으로 생성하도록 수정 ✅
```python
# 수정 후
...
**상태**: 코드 수정 완료, 백엔드 재시작 완료
**다음 단계**: 프론트엔드에서 전략 재저장 → 백테스트 실행 테스트

### 📋 다음 작업 (2025-12-09)
1. 전략 ID 3 재저장 (새 코드로 DB 업데이트)
2. DB 확인 (syntax error 해결 확인)
3. 포트폴리오 백테스트 실행 테스트
4. 결과 검증 (종목 자동 선정, 수익률 계산)

**상세 작업 내용**: `TODO_TOMORROW.md` 참고

---

**테스트 결과:**
- 전략 빌더 저장/수정: 동작 확인 필요
- 포트폴리오 백테스트: syntax error로 대기 중
- 재무 데이터 수집: 정상 동작 ✅
