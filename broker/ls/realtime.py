...
class LSRealtimeService:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            ws_url = self.ws_url  # LSEndpoints.get_wss_url()로 설정된 URL 사용
    ...
    async def subscribe(self, symbols: List[str]) -> None:
        ...
        'S3_' (주식체결) TR과 'JIF' (장운영정보) TR을 구독합니다.
        ...
        logger.info(f"Subscribed to {subscribed_count} symbols (S3_) and JIF")
    ...
    async def stream(self, symbols: List[str]) -> AsyncIterator[Dict[str, Any]]:
        ...
        Yields:
            실시간 가격 데이터 {'symbol', 'price', 'status', 'timestamp'}
        """
        await self.connect()
